		...
		zend_string **opened_path, php_stream_context *context STREAMS_DC)
{
	...
	if ((strlen(path) > 2) && (path[0] != '/') && (path[1] != ':') && (!wrapper->is_url) && (PG(type_strictness))) {
		php_error_docref(NULL, E_WARNING, "relative paths are not allowed in strict development mode");
	}
	...
	if (stream && (strchr(mode,'w')) && (strstr(path, "php://") == NULL))
	{
		// We are creating a file, so it should have fix_permissions called for it - record it as created
		zval tmp, *arr;
		ZVAL_STRING(&tmp, path);
		arr = &PG(http_globals)[TRACK_VARS_CREATED_FILES];
		zend_hash_next_index_insert(Z_ARRVAL_P(arr), &tmp);
	}

	if (stream && ((strchr(mode,'a')) || (strchr(mode,'w'))) && (strstr(path, "php://") == NULL))
	{
		// We are writing a file, so it should have sync_file called for it - record it as modified
		zval tmp, *arr;
		ZVAL_STRING(&tmp, path);
		arr = &PG(http_globals)[TRACK_VARS_MODIFIED_FILES];
		zend_hash_next_index_insert(Z_ARRVAL_P(arr), &tmp);
	}
