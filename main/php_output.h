...
/* convenience macros */
#define PHPWRITE(str, str_len)		php_output_write((str), (str_len), 0)
#define PHPWRITE_XSS(str, str_len, escaped)		php_output_write((str), (str_len), escaped)
#define PHPWRITE_H(str, str_len)	php_output_write_unbuffered((str), (str_len), 0)
#define PHPWRITE_H_XSS(str, str_len, escaped)	php_output_write_unbuffered((str), (str_len), escaped)

#define PUTC(c)						php_output_write((const char *) &(c), 1, 0)
#define PUTC_XSS(c, escaped)						php_output_write((const char *) &(c), 1, escaped)
#define PUTC_H(c)					php_output_write_unbuffered((const char *) &(c), 1, 0)
#define PUTC_H_XSS(c, escaped)					php_output_write_unbuffered((const char *) &(c), 1, escaped)

#define PUTS(str)					do {				\
	const char *__str = (str);							\
	php_output_write(__str, strlen(__str), 0);	\
} while (0)
#define PUTS_XSS(str, escaped)					do {				\
	const char *__str = (str);							\
	php_output_write(__str, strlen(__str), escaped);	\
} while (0)
#define PUTS_H(str)					do {							\
	const char *__str = (str);										\
	php_output_write_unbuffered(__str, strlen(__str), 0);	\
} while (0)
#define PUTS_H_XSS(str, escaped)					do {							\
	const char *__str = (str);										\
	php_output_write_unbuffered(__str, strlen(__str), escaped);	\
} while (0)
