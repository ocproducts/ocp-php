...
/*
Our XSS system would detect issues when writing into the buffer, if enabled.
It also tracks XSS status within the buffer, so we can continue to track the string that comes out of it (ob_get_contents etc).
This way we can choose to disable XSS detection while writing into the buffer, if we just want a string. But then when that string is output, we run (compound) checking again.
*/
...
static inline void php_output_op(int op, const char *str, size_t len, int escaped);
 ...
 * Unbuffered write */
PHPAPI size_t php_output_write_unbuffered(const char *str, size_t len, int escaped)
{
	if (OG(flags) & PHP_OUTPUT_ACTIVATED) {
 ...
 * Buffered write */
PHPAPI size_t php_output_write(const char *str, size_t len, int escaped)
{
	//raise(SIGINT);

	if (OG(flags) & PHP_OUTPUT_ACTIVATED) {
		php_output_op(PHP_OUTPUT_HANDLER_WRITE, str, len, escaped);
		return len;
	}
...
PHPAPI int php_output_flush(void)
{
	...
	if (OG(active) && (OG(active)->flags & PHP_OUTPUT_HANDLER_FLUSHABLE)) {
		...
		if (context.out.data && context.out.used) {
			zend_stack_del_top(&OG(handlers));
			php_output_write(context.out.data, context.out.used, context.out.escaped);
...
{
	if (OG(active)) {
		php_output_op(PHP_OUTPUT_HANDLER_FLUSH, NULL, 0, 1);
	}
}
/* }}} */
...
{
	if (OG(active)) {
		ZVAL_STRINGL_XSS(p, OG(active)->buffer.data, OG(active)->buffer.used, OG(active)->buffer.escaped);
		return SUCCESS;
	} else {
		ZVAL_NULL(p);
		return FAILURE;
...
static inline php_output_handler_status_t php_output_handler_op(php_output_handler *handler, php_output_context *context)
{
	...
	if (php_output_handler_append(handler, &context->in) && !context->op) {
		...
		if (handler->flags & PHP_OUTPUT_HANDLER_USER) {
			...
			ZVAL_STRINGL_XSS(&ob_data, handler->buffer.data, handler->buffer.used, handler->buffer.escaped);
