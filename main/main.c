...
PHP_INI_BEGIN()
	...
	STD_PHP_INI_BOOLEAN("ocproducts.xss_detect",	"0",	PHP_INI_ALL,		OnUpdateBool,			xss_detect,	php_core_globals,	core_globals)
	STD_PHP_INI_BOOLEAN("ocproducts.type_strictness",	"0",	PHP_INI_ALL,		OnUpdateBool,			type_strictness,	php_core_globals,	core_globals)
...
PHPAPI ZEND_COLD void php_verror(const char *docref, const char *params, int type, const char *format, va_list args)
{
	...
	if (PG(track_errors) && module_initialized && EG(active) &&
		...
		zval tmp;
		ZVAL_STRINGL_XSS(&tmp, buffer, buffer_len, 1);
...
/* {{{ php_output_wrapper
 */
static size_t php_output_wrapper(const char *str, size_t str_length, int escaped)
{
	return php_output_write(str, str_length, escaped);
}
/* }}} */
