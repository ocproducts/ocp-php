...
static inline void zend_insert_literal(zend_op_array *op_array, zval *zv, int literal_position) /* {{{ */
{
	...
	if (Z_TYPE_P(zv) == IS_STRING) {
		...
		if (Z_TYPE_P(zv) == IS_STRING) {
			// zend_new_interned_string (zend_new_interned_string_int) may have removed escaped = 1 because it is re-using some existing string, e.g. a string from the engine (the string "test" does this)
			//  --> so we must put it in here
			(*zv).value.str->escaped = 1;
		}
	}
...
static inline zend_bool zend_try_ct_eval_binary_op(zval *result, uint32_t opcode, zval *op1, zval *op2) /* {{{ */
{
	...
	if ((opcode == ZEND_DIV || opcode == ZEND_MOD)) {
		if ((Z_TYPE_P(op2) == IS_LONG) && (zval_get_long(op2) == 0)) return 0;
		if ((Z_TYPE_P(op2) == IS_DOUBLE) && (zval_get_double(op2) == 0)) return 0;
