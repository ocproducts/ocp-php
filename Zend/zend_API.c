...
{
	if (Z_TYPE_P(arg) != IS_TRUE && Z_TYPE_P(arg) != IS_FALSE && Z_TYPE_P(arg) != _IS_BOOL) {
		ocp_type_strictness_error("zend_parse_arg_bool_weak");
	}
...
{
	if (Z_TYPE_P(arg) != IS_LONG && Z_TYPE_P(arg) != IS_NULL) {
		ocp_type_strictness_error("zend_parse_arg_long_weak");
	}
...
{
	if (Z_TYPE_P(arg) != IS_LONG && Z_TYPE_P(arg) != IS_NULL) {
		ocp_type_strictness_error("zend_parse_arg_long_cap_weak");
	}
...
{
	if (Z_TYPE_P(arg) != IS_DOUBLE && Z_TYPE_P(arg) != IS_NULL) {
		ocp_type_strictness_error("zend_parse_arg_double_weak");
	}
...
{
	if (Z_TYPE_P(arg) != IS_DOUBLE && Z_TYPE_P(arg) != IS_NULL) {
		ocp_type_strictness_error("zend_parse_arg_double_slow");
	}
...
{
	/*if (Z_TYPE_P(arg) != IS_STRING && Z_TYPE_P(arg) != IS_NULL) {		convert_to_string actually shows this error message
		ocp_type_strictness_error("zend_parse_arg_str_weak");
	}*/
...
ZEND_API int add_assoc_string_ex_xss(zval *arg, const char *key, size_t key_len, const char *str, int escaped) /* {{{ */
{
	...
	ZVAL_STRING_XSS(&tmp, str, escaped);
	zend_symtable_str_update(Z_ARRVAL_P(arg), key, key_len, &tmp);
	return SUCCESS;
}
/* }}} */
...
{
	return add_assoc_string_ex_xss(arg, key, key_len, str, 0);
}
/* }}} */
...
ZEND_API int add_assoc_stringl_ex_xss(zval *arg, const char *key, size_t key_len, const char *str, size_t length, int escaped) /* {{{ */
{
	...
	ZVAL_STRINGL_XSS(&tmp, str, length, escaped);
	zend_symtable_str_update(Z_ARRVAL_P(arg), key, key_len, &tmp);
	return SUCCESS;
}
/* }}} */
...
{
	return add_assoc_stringl_ex_xss(arg, key, key_len, str, length, 0);
}
/* }}} */
...
ZEND_API int add_index_string_xss(zval *arg, zend_ulong index, const char *str, int escaped) /* {{{ */
{
	zval tmp;
...
{
	return add_index_string_xss(arg, index, str, 0);
}
/* }}} */

ZEND_API int add_index_stringl_xss(zval *arg, zend_ulong index, const char *str, size_t length, int escaped) /* {{{ */
{
	zval tmp;
...
{
	return add_index_stringl_xss(arg, index, str, length, 0);
}
/* }}} */
...
{
	return add_next_index_string_xss(arg, str, 0);
}
/* }}} */
...
{
	return add_next_index_stringl_xss(arg, str, length, 0);
}
/* }}} */
...
{
	return add_get_assoc_string_ex_xss(arg, key, key_len, str, 0);
}
/* }}} */
...
{
	return add_get_assoc_stringl_ex_xss(arg, key, key_len, str, length, 0);
}
/* }}} */
...
ZEND_API zval *add_get_index_string(zval *arg, zend_ulong index, const char *str) /* {{{ */
{
	...
	ZVAL_STRING_XSS(&tmp, str, (*arg).value.str->escaped);
	return zend_hash_index_update(Z_ARRVAL_P(arg), index, &tmp);
}
/* }}} */
...
ZEND_API zval *add_get_index_stringl(zval *arg, zend_ulong index, const char *str, size_t length) /* {{{ */
{
	...
	ZVAL_STRINGL_XSS(&tmp, str, length, (*arg).value.str->escaped);
	return zend_hash_index_update(Z_ARRVAL_P(arg), index, &tmp);
}
/* }}} */
...
{
	return add_property_string_ex_xss(arg, key, key_len, str, 0);
}
/* }}} */
...
{
	return add_property_stringl_ex_xss(arg, key, key_len, str, length, 0);
}
/* }}} */
