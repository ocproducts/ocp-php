...
static void print_hash(smart_str *buf, HashTable *ht, int indent, zend_bool is_object) /* {{{ */
{
	...
	smart_str_appends_xss(buf, "(\n", 1);
	indent += PRINT_ZVAL_INDENT;
	ZEND_HASH_FOREACH_KEY_VAL_IND(ht, num_key, string_key, tmp) {
		for (i = 0; i < indent; i++) {
			smart_str_appendc_xss(buf, ' ', 1);
		}
		smart_str_appendc_xss(buf, '[', 1);
		if (string_key) {
			if (is_object) {
				...
				if (class_name && mangled == SUCCESS) {
					if (class_name[0] == '*') {
						smart_str_appends_xss(buf, ":protected", 1);
					} else {
						smart_str_appends_xss(buf, ":", 1);
						smart_str_appends_xss(buf, class_name, 1);
						smart_str_appends_xss(buf, ":private", 1);
					}
				}
			} else {
				smart_str_append_xss(buf, string_key, 1);
			}
		} else {
...
static void print_flat_hash(HashTable *ht) /* {{{ */
{
	...
	ZEND_HASH_FOREACH_KEY_VAL_IND(ht, num_key, string_key, tmp) {
		...
		ZEND_PUTS("[", 1);
		if (string_key) {
			ZEND_WRITE(ZSTR_VAL(string_key), ZSTR_LEN(string_key), 1);
		} else {
...
ZEND_API int zend_make_printable_zval(zval *expr, zval *expr_copy) /* {{{ */
{
	if (Z_TYPE_P(expr) == IS_STRING) {
		...
		switch (Z_TYPE_P(expr)) {
			...
			case IS_OBJECT:
			case IS_REFERENCE:
				expr_copy->value.str->escaped = 1;
				break;
			case IS_STRING:
				// As copied
				break;
			default:
				expr_copy->value.str->escaped = 0;
				break;
		}
		return 1;
	}
...
void ocproducts_check_for_xss(zend_string *str)
{
	if (str->escaped == 0)
	{
		if (PG(xss_detect))
		{
			char *_str1, *_str2, *_str;

			//raise(SIGINT);

			_str1 = "XSS vulnerability: ";
			_str2 = str->val;
			_str = (char *)calloc(strlen(_str1) + strlen(_str2) + 1, sizeof(char));
			strcat(_str, _str1);
			strcat(_str, _str2);
	
			zend_error(E_WARNING, _str);
		}
	}
}
...
ZEND_API size_t zend_print_zval(zval *expr, int indent) /* {{{ */
{
	...
	if (len != 0) {
		ocproducts_check_for_xss(str);
		zend_write(ZSTR_VAL(str), len, str->escaped);
	}
...
static void zend_print_zval_r_to_buf(smart_str *buf, zval *expr, int indent) /* {{{ */
{
	...
	switch (Z_TYPE_P(expr)) {
		case IS_ARRAY:
			smart_str_appends_xss(buf, "Array\n", 1);
			if (!(GC_FLAGS(Z_ARRVAL_P(expr)) & GC_IMMUTABLE)) {
				if (GC_IS_RECURSIVE(Z_ARRVAL_P(expr))) {
					smart_str_appends_xss(buf, " *RECURSION*", 1);
					return;
				}
				GC_PROTECT_RECURSION(Z_ARRVAL_P(expr));
			}
		...
		case IS_OBJECT:
			{
				...
				zend_string *class_name = Z_OBJ_HANDLER_P(expr, get_class_name)(Z_OBJ_P(expr));
				smart_str_appends_xss(buf, ZSTR_VAL(class_name), 1);
				zend_string_release_ex(class_name, 0);

				smart_str_appends_xss(buf, " Object\n", 1);
				if (GC_IS_RECURSIVE(Z_OBJ_P(expr))) {
					smart_str_appends_xss(buf, " *RECURSION*", 1);
					return;
				}
...
ZEND_API void zend_print_zval_r(zval *expr, int indent) /* {{{ */
{
	...
	ocproducts_check_for_xss(str);
	zend_write(ZSTR_VAL(str), ZSTR_LEN(str), str->escaped);
	zend_string_release_ex(str, 0);
}
/* }}} */
