...
#define smart_str_appends_ex_xss(dest, src, what, escaped) \
	smart_str_appendl_ex_xss((dest), (src), strlen(src), (what), escaped)
#define smart_str_appends(dest, src) \
	smart_str_appendl((dest), (src), strlen(src))
#define smart_str_appends_xss(dest, src, escaped) \
	smart_str_appendl_xss((dest), (src), strlen(src), escaped)
#define smart_str_extend(dest, len) \
	smart_str_extend_ex((dest), (len), 0)
#define smart_str_appendc(dest, c) \
	smart_str_appendc_ex((dest), (c), 0)
#define smart_str_appendc_xss(dest, c, escaped) \
	smart_str_appendc_ex_xss((dest), (c), 0, escaped)
#define smart_str_appendl(dest, src, len) \
	smart_str_appendl_ex((dest), (src), (len), 0)
#define smart_str_appendl_xss(dest, src, len, escaped) \
	smart_str_appendl_ex_xss((dest), (src), (len), 0, escaped)
#define smart_str_append(dest, src) \
	smart_str_append_ex((dest), (src), 0)
#define smart_str_append_xss(dest, src, escaped) \
	smart_str_append_ex_xss((dest), (src), 0, escaped)
#define smart_str_append_smart_str(dest, src) \
	smart_str_append_smart_str_ex((dest), (src), 0)
#define smart_str_append_smart_str_xss(dest, src, escaped) \
	smart_str_append_smart_str_ex_xss((dest), (src), 0, escaped)
...
static zend_always_inline void smart_str_appendc_ex_xss(smart_str *dest, char ch, zend_bool persistent, int escaped) {
	size_t new_len = smart_str_alloc(dest, 1, persistent);
	ZSTR_VAL(dest->s)[new_len - 1] = ch;
	ZSTR_LEN(dest->s) = new_len;
	if (escaped == 0) dest->s->escaped = 0;
}

static zend_always_inline void smart_str_appendc_ex(smart_str *dest, char ch, zend_bool persistent) {
	smart_str_appendc_ex_xss(dest, ch, persistent, 0);
}

static zend_always_inline void smart_str_appendl_ex_xss(smart_str *dest, const char *str, size_t len, zend_bool persistent, int escaped) {
	size_t new_len = smart_str_alloc(dest, len, persistent);
...
static zend_always_inline void smart_str_appendl_ex(smart_str *dest, const char *str, size_t len, zend_bool persistent) {
	smart_str_appendl_ex_xss(dest, str, len, persistent, 0);
}

static zend_always_inline void smart_str_append_ex_xss(smart_str *dest, const zend_string *src, zend_bool persistent, int escaped) {
	smart_str_appendl_ex_xss(dest, ZSTR_VAL(src), ZSTR_LEN(src), persistent, escaped);
}

static zend_always_inline void smart_str_append_ex(smart_str *dest, const zend_string *src, zend_bool persistent) {
	smart_str_append_ex_xss(dest, src, persistent, 0);
}

static zend_always_inline void smart_str_append_smart_str_ex_xss(smart_str *dest, const smart_str *src, zend_bool persistent, int escaped) {
	if (src->s && ZSTR_LEN(src->s)) {
		smart_str_append_ex_xss(dest, src->s, persistent, escaped);
	}
}

static zend_always_inline void smart_str_append_smart_str_ex(smart_str *dest, const smart_str *src, zend_bool persistent) {
	smart_str_append_smart_str_ex_xss(dest, src, persistent, 0);
}
