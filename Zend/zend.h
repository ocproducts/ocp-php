...
typedef struct _zend_utility_functions {
	...
	size_t (*write_function)(const char *str, size_t str_length, int escaped);
...
typedef int (*zend_write_func_t)(const char *str, size_t str_length, int escaped);
...
/* output support */
#define ZEND_WRITE(str, str_len, escaped)		zend_write((str), (str_len), escaped)
#define ZEND_WRITE_EX(str, str_len, escaped)		write_func((str), (str_len), escaped)
#define ZEND_PUTS(str, escaped)					zend_write((str), strlen((str)), escaped)
#define ZEND_PUTS_EX(str, escaped)				write_func((str), strlen((str)), escaped)
#define ZEND_PUTC(c, escaped)					zend_write(&(c), 1, escaped)
