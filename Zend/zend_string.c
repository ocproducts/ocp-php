...
ZEND_API void zend_interned_strings_init(void)
{
	...
	str = zend_string_alloc_xss(sizeof("")-1, 1, 1);
	ZSTR_VAL(str)[0] = '\000';
	...
	for (i = 0; i < 256; i++) {
		...
		zend_one_char_string[i]->escaped = 0; // Has to be set here, because if it was already set as a static string in code it would come from that, marked escaped
	}
