...
ZEND_API void zend_lex_tstring(zval *zv)
{
	...
	ZVAL_STRINGL_XSS(zv, (char*)SCNG(yy_text), SCNG(yy_leng), 1);
}
...
# define zend_copy_value(zendlval, yytext, yyleng) \
	if (SCNG(output_filter)) { \
		...
		ZVAL_STRINGL_XSS(zendlval, s, sz, 1); \
		efree(s); \
	} else if (yyleng == 1) { \
		ZVAL_INTERNED_STR(zendlval, ZSTR_CHAR((zend_uchar)*(yytext))); \
	} else { \
		ZVAL_STRINGL_XSS(zendlval, yytext, yyleng, 1); \
	}
...
static int zend_scan_escape_string(zval *zendlval, char *str, int len, char quote_type)
{
	...
	ZVAL_STRINGL_XSS(zendlval, str, len, 1);
...
skip_escape_conversion:
	if (SCNG(output_filter)) {
		...
		zval_ptr_dtor(zendlval);
		ZVAL_STRINGL_XSS(zendlval, (char *) str, sz, 1);
		efree(str);
	}
	return SUCCESS;
}
...
#line 2133 "Zend/zend_language_scanner.l"
	...
	ZVAL_STRINGL_XSS(zendlval, yytext+bprefix+1, yyleng-bprefix-2, 1);
...
skip_escape_conversion:
	if (SCNG(output_filter)) {
		...
		ZVAL_STRINGL_XSS(zendlval, str, sz, 1);
	}
	RETURN_TOKEN_WITH_VAL(T_CONSTANT_ENCAPSED_STRING);
}
...
heredoc_scan_done:
	...
	yyleng = YYCURSOR - SCNG(yy_text);
	ZVAL_STRINGL_XSS(zendlval, yytext, yyleng - newline, 1);
