...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_STRING:
			{
				ocp_type_strictness_error("_convert_scalar_to_number");
...
static zend_always_inline zval* _zendi_convert_scalar_to_number_ex(zval *op, zval *holder, zend_bool silent) /* {{{ */
{
	switch (Z_TYPE_P(op)) {
		case IS_NULL:
		case IS_FALSE:
			ocp_type_strictness_error("_zendi_convert_scalar_to_number_ex");
			ZVAL_LONG(holder, 0);
			return holder;
		case IS_TRUE:
			ocp_type_strictness_error("_zendi_convert_scalar_to_number_ex");
...
ZEND_API void ZEND_FASTCALL convert_to_long(zval *op) /* {{{ */
{
	if (Z_TYPE_P(op) != IS_LONG) {
		ocp_type_strictness_error("convert_to_long");
...
try_again:
	switch (Z_TYPE_P(op)) {
		case IS_NULL:
		case IS_FALSE:
			ocp_type_strictness_error("convert_to_long_base");
			ZVAL_LONG(op, 0);
			break;
		case IS_TRUE:
			ocp_type_strictness_error("convert_to_long_base");
...
try_again:
	switch (Z_TYPE_P(op)) {
		case IS_NULL:
		case IS_FALSE:
			ocp_type_strictness_error("convert_to_double");
			ZVAL_DOUBLE(op, 0.0);
			break;
		case IS_TRUE:
			ocp_type_strictness_error("convert_to_double");
...
{
	if (Z_TYPE_P(op) != IS_NULL) {
		ocp_type_strictness_error("convert_to_null");
	}
...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_TRUE:
			break;
		case IS_NULL:
			ocp_type_strictness_error("convert_to_boolean");
			ZVAL_FALSE(op);
			break;
		case IS_RESOURCE: {
...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_NULL:
		case IS_FALSE: {
			ocp_type_strictness_error("_convert_to_string");
			ZVAL_EMPTY_STRING(op);
			break;
		}
		case IS_TRUE:
		...
		case IS_ARRAY:
			...
			zval_ptr_dtor(op);
			ZVAL_NEW_STR(op, zend_string_init_xss("Array", sizeof("Array")-1, 0, 1));
			break;
		case IS_OBJECT: {
			zval dst;
			...
			if (Z_TYPE(dst) == IS_STRING) {
				...
				ZVAL_NEW_STR(op, zend_string_init_xss("Object", sizeof("Object")-1, 0, 1));
			}
			break;
		}
		case IS_REFERENCE:
...
{
	ocp_type_strictness_error("convert_scalar_to_array");
...
/* OBJECTS_OPTIMIZE */
		case IS_OBJECT:
			ocp_type_strictness_error("convert_to_array");
		...
		case IS_ARRAY:
			{
				...
				ocp_type_strictness_error("convert_to_object");
				zend_object *obj;
...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_NULL:
		case IS_FALSE:
			if (strict == 1) ocp_type_strictness_error("_zval_get_long_func");
			return 0;
		case IS_TRUE:
			if (strict == 1) ocp_type_strictness_error("_zval_get_long_func");
			return 1;
...
try_again:
	switch (Z_TYPE_P(op)) {
		case IS_NULL:
		case IS_FALSE:
			if (strict == 1) ocp_type_strictness_error("_zval_get_double_func");
			return 0.0;
		case IS_TRUE:
			if (strict == 1) ocp_type_strictness_error("_zval_get_double_func");
			return 1.0;
...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_NULL:
		case IS_FALSE:
			if (strict == 1) ocp_type_strictness_error("_zval_get_string_func");
			return ZSTR_EMPTY_ALLOC();
		case IS_TRUE:
			if (strict == 1) ocp_type_strictness_error("_zval_get_string_func");
			return ZSTR_CHAR('1');
		...
		case IS_DOUBLE: {
			zend_string *ret;
			if (strict == 1) ocp_type_strictness_error("_zval_get_string_func");
			ret = zend_strpprintf(0, "%.*G", (int) EG(precision), Z_DVAL_P(op));
			ret->escaped = 1;
			return ret;
		}
		case IS_ARRAY:
			if (strict == 1) ocp_type_strictness_error("_zval_get_string_func");
			zend_error(E_NOTICE, "Array to string conversion");
			return zend_string_init_xss("Array", sizeof("Array")-1, 0, 1);
		case IS_OBJECT: {
			if (strict == 1) ocp_type_strictness_error("_zval_get_string_func");
			zval tmp;
			if (Z_OBJ_HT_P(op)->cast_object) {
...
ZEND_API int ZEND_FASTCALL concat_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (UNEXPECTED(Z_STRLEN_P(op1) == 0)) {
		...
		int escaped = ((Z_TYPE_P(op1) != IS_STRING || ((*op1).value.str != NULL && (*op1).value.str->escaped == 1)) && (Z_TYPE_P(op2) != IS_STRING || ((*op2).value.str != NULL && (*op2).value.str->escaped == 1)));
		...
		result_str->escaped = escaped ? 1 : 0;
...
ZEND_API int ZEND_FASTCALL numeric_compare_function(zval *op1, zval *op2) /* {{{ */
{
	...
	if ((Z_TYPE_P(op1) == IS_STRING || Z_TYPE_P(op2) == IS_STRING) && PG(type_strictness)) {
		ocp_type_strictness_error("numeric_compare_function");
	}

	d1 = zval_get_double_func_ex(op1, 0);
	d2 = zval_get_double_func_ex(op2, 0);
	...
	while (1) {
		switch (TYPE_PAIR(Z_TYPE_P(op1), Z_TYPE_P(op2))) {
			...
			default:
				...
				if ( ((Z_TYPE_P(op1) != IS_NULL) && (Z_TYPE_P(op2) != IS_NULL) && (Z_TYPE_P(op1) != Z_TYPE_P(op2))) && PG(type_strictness)) {
					if (((Z_TYPE_P(op1) == IS_FALSE) || (Z_TYPE_P(op1) == IS_TRUE)) && ((Z_TYPE_P(op2) == IS_FALSE) || (Z_TYPE_P(op2) == IS_TRUE))) {
					} else {
						ocp_type_strictness_error("compare_function");
					}
				}
...
try_again:
	switch (Z_TYPE_P(op1)) {
		...
		case IS_DOUBLE:
			...
			break;
		case IS_NULL:
			ocp_type_strictness_error("increment_function");
...
ZEND_API zend_string* ZEND_FASTCALL zend_long_to_str(zend_long num) /* {{{ */
{
	if ((zend_ulong)num <= 9) {
		...
		return zend_string_init_xss(res, buf + sizeof(buf) - 1 - res, 0, 1);
	}
}
/* }}} */
...
void ocp_type_strictness_error(const char* source)
{
	if (PG(type_strictness) == 1) {
		...
		char msg[200];
		sprintf(msg, "type strictness error (%s)", source);
