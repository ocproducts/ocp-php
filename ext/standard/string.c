...
static zend_always_inline zend_string *php_trim_int(zend_string *str, char *what, size_t what_len, int mode)
{
	...
	if (ZSTR_LEN(str) == end - start) {
		...
		return zend_string_init_xss(start, end - start, 0, standard_trim ? str->escaped : 0);
	}
}
/* }}} */
	...
	if (p2 == NULL) {
		...
		do {
			...
			if (l == 0) {
				...
				ZVAL_STRINGL_XSS(&tmp, p1, p2 - p1, str->escaped);
			}
			zend_hash_next_index_insert_new(Z_ARRVAL_P(return_value), &tmp);
		...
		if (p1 <= endp) {
			ZVAL_STRINGL_XSS(&tmp, p1, endp - p1, str->escaped);
			zend_hash_next_index_insert_new(Z_ARRVAL_P(return_value), &tmp);
		}
	}
}
	...
	if (p2 == NULL) {
		...
		*/
		...
		for (i = 0; i < to_return; i++) { /* this checks also for to_return > 0 */
			ZVAL_STRINGL_XSS(&tmp, positions[i], (positions[i+1] - ZSTR_LEN(delim)) - positions[i], str->escaped);
			zend_hash_next_index_insert_new(Z_ARRVAL_P(return_value), &tmp);
		}
...
PHP_FUNCTION(chr)
{
	...
	c &= 0xff;
	zend_one_char_string[c]->escaped = 0; // Not sure why this needed TBH, but it is! zend_one_char_string is always set with it being 0. Perhaps the values are overwritten in some subtle way
...
static zend_string* php_ucfirst(zend_string *str)
{
	...
	if (r == ZSTR_VAL(str)[0]) {
		...
		zend_string *s = zend_string_init_xss(ZSTR_VAL(str), ZSTR_LEN(str), 0, str->escaped);
		ZSTR_VAL(s)[0] = r;
		return s;
	}
}
...
nothing_todo:
		return zend_string_dup(haystack, 0);
	} else {
		if (str_len == 0) {
...
PHP_FUNCTION(stripcslashes)
{
	...
	ZVAL_STRINGL_XSS(return_value, ZSTR_VAL(str), ZSTR_LEN(str), str->escaped);
...
PHP_FUNCTION(str_pad)
{
	...
	result = zend_string_safe_alloc_xss(1, ZSTR_LEN(input), num_pad_chars, 0, input->escaped);
	ZSTR_LEN(result) = 0;
