...
static int php_info_print_html_esc(const char *str, size_t len) /* {{{ */
{
	...
	written = php_output_write(ZSTR_VAL(new_str), ZSTR_LEN(new_str), 1);
...
static int php_info_printf(const char *fmt, ...) /* {{{ */
{
	...
	written = php_output_write(buf, len, 1);
	efree(buf);
	return written;
}
/* }}} */
...
{
	return php_output_write(str, strlen(str), 1);
}
/* }}} */
...
PHP_FUNCTION(phpinfo)
{
	...
	bak_ts = PG(type_strictness);
	PG(type_strictness) = 0;

	bak_xss = PG(xss_detect);
	PG(xss_detect) = 0;
	...
	PG(type_strictness) = bak_ts;

	PG(xss_detect) = bak_xss;
