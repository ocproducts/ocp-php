...
/* {{{ preg_do_repl_func
 */
static zend_string *preg_do_repl_func(zend_fcall_info *fci, zend_fcall_info_cache *fcc, char *subject, PCRE2_SIZE *offsets, char **subpat_names, int count, const PCRE2_SPTR mark, int escaped)
{
	...
	if (zend_call_function(fci, fcc) == SUCCESS && Z_TYPE(retval) != IS_UNDEF) {
		...
		result_str = zend_string_init_xss(&subject[offsets[0]], offsets[1] - offsets[0], 0, escaped);
	}
...
matched:
			...
			if (new_len >= alloc_len) {
				...
				if (result == NULL) {
					result = zend_string_alloc_xss(alloc_len, 0, subject_str->escaped);
				} else {
...
matched:
			...
			eval_result = preg_do_repl_func(fci, fcc, subject, offsets, subpat_names, count,
				pcre2_get_mark(match_data), subject_str->escaped);
